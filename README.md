...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isCommand() && interaction.isCommand()) {
      ...
      const EMBED_COLOR = "#FFA500"; // orange
      const FOOTER_TEXT = "Powered by All In One Bot";
      ...
      const createPages = (commandSet, type) => {
        ...
        for (const [category, commands] of Object.entries(commandSet)) {
          ...
          const totalSubcommands = commands.reduce((acc, cmd) => acc + (cmd.subcommands?.length || 0), 0);
          ...
          if (joinedCommands.length > 1024) {
            ...
            for (const line of page.commands) {
              if (fieldValue.length + line.length + 2 > 1024) {
                fields.push({ name: `Command List (Part ${fieldCount})`, value: fieldValue.trim() });
                fieldCount++;
                fieldValue = line + '\n\n';
              } else {
                fieldValue += line + '\n\n';
          ...
          const disabledComponents = createComponents().map(row => {
            ...
            row.components.forEach(component => {
              if (component.data.type === 2) {
                updatedRow.addComponents(ButtonBuilder.from(component.data).setDisabled(true));
              } else if (component.data.type === 3) {
                updatedRow.addComponents(StringSelectMenuBuilder.from(component.data).setDisabled(true));
              }
            });
            return updatedRow;
          });
